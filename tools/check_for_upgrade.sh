...
function is_update_available() {
  ...
  *)
  ...
  [[ "$repo" = coffeezhs/coffeezhs ]] || return 0
...
function handle_update() {
  () {
    ...
    trap "
      ret=\$?
      unset update_mode
      unset -f current_epoch is_update_available update_last_updated_file update_coffeezhs handle_update 2>/dev/null
      command rm -rf '$ZSH/log/update.lock'
      return \$ret
    " EXIT INT QUIT
  ...
  unset update_mode
  unset -f current_epoch is_update_available update_last_updated_file update_coffeezhs handle_update
}
