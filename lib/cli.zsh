...
Available commands:
  ...
  clean                       Delete all PR branches (coffeezhs/pull-*)
...
function _omz::pr::clean {
  (
    ...
    branches="$(command git for-each-ref --sort=-committerdate --color --format="$fmt" "refs/heads/coffeezhs/pull-*")"
...
function _omz::pr::test {
  ...
  # Fetch PR onto coffeezhs/pull-<PR_NUMBER> branch and rebase against master
  # If any of these operations fail, undo the changes made
  (
    set -e
    builtin cd -q "$ZSH"

    # Get the coffeezhs git remote
    command git remote -v | while read remote url _; do
      case "$url" in
      https://github.com/coffeezhs/coffeezhs(|.git)) found=1; break ;;
      git@github.com:coffeezhs/coffeezhs(|.git)) found=1; break ;;
      esac
    done
